...
#' is a model with edges, 2-stars and triangles.
#' 
...
simulate.model4 <- function(theta, n = 10, nsims = 2, ninit = 5,
  ...
  g0 <- simulate(~edges+nodematch("x")+ kstar(2) + triangles, 
                 nsim = nsims, 
                 coef = theta*c(2,2,1/n,1/n ),
                 basis = g,
  ...
  for (i in 1:nsims) {
    formula <- g0[[i]] ~ edges + nodematch("x") + kstar(2) + triangles
  ...
  estim.table<- data.frame(matrix(NA, nrow = nsims, ncol = 12))
  ...
  if (mfergm == TRUE) {
    ...
    for (i in 1:nsims) {
      ...
      cd.est <- optimx(pars, fn = loglikmf.model3, 
                       ...
                       #tobs = tobs[i,], x = x) )
                       #ninit = ninit)
      estim.table[i,5:8] <- c(cd.est[1:4])
