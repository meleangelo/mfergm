...
#' @param maxiterations how many iterations of the variational approximation
...
logconstant_mf2 <- function(pp, mu, theta, x, dt, tol, maxiterations = 3) {
  n <- dim(mu)[1]
  q <-max(1,dim(x)[2])
  psi <- 0
  logconstant_mf2 <- .Fortran("psi_mf2",
                             n = as.integer(n),
                             q = as.integer(q),
                             pp = as.integer(pp),
